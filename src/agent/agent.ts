  ...
  async handleMessage(source, message, max_responses = null) {
    ...
    if (!self_prompt && !from_other_bot) {
      ...
      if (user_command_name) {
        ...
        const executeResult = await executeCommand(this, message);
        if (executeResult) {
    ...
    for (let index = 0; index < max_responses; index++) {
      ...
      const command_name = containsCommand(response);
      ...
      if (command_name) {
        ...
        const executeResult = await executeCommand(this, response);
