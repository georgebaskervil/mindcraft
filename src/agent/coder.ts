...
export class Coder {
  ...
  async stageCode(code) {
    ...
    const source_lint_copy = this.code_lint_template.replace(
      "/* CODE HERE */",
      source,
    );
    ...
    const writeResult = await this.writeFilePromise(
      "." + this.fp + filename,
      source,
    );
  ...
  async generateCode(agent_history) {
    ...
    const result = await this.generateCodeLoop(agent_history);
    this.generating = false;
  ...
  async generateCodeLoop(agent_history) {
    ...
    for (let index = 0; index < 5; index++) {
      ...
      const response = await this.agent.prompter.promptCoding(
        structuredClone(messages),
      );
      if (this.agent.bot.interrupt_code) {
        return interrupt_return;
      }
      const contains_code = response.includes("```");
      if (!contains_code) {
      ...
      const source_lint_copy = result.src_lint_copy;
