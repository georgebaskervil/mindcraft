  ...
  for (const empty_po of empty_pos) {
    ...
    for (let x = 0; x < size; x++) {
      for (let z = 0; z < size; z++) {
        const top = bot.blockAt(empty_po.offset(x, 0, z));
        const bottom = bot.blockAt(empty_po.offset(x, -1, z));
        if (
          !top ||
...
export function getSurroundingBlocks(bot) {
  ...
  const surroundingBlocks = []; // Renamed from res
...
export function getFirstBlockAboveHead(
  ...
  if (ignore_types === null) {
    ...
    for (const ignore_type of ignore_types) {
...
export function getNearestBlocks(
  ...
  const results = []; // Renamed from res
...
export function getCraftableItems(bot) {
  ...
  const craftableItems = []; // Renamed from res
