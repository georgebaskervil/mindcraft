...
export async function craftRecipe(bot, itemName, number_ = 1) {
  ...
  placeTable: if (!recipes || recipes.length === 0) {
    ...
    if (!recipes || recipes.length === 0) {
      break placeTable;
    } //Don't bother going to the table if we don't have the required resources.
...
export async function clearNearestFurnace(bot) {
  ...
  const smelted_name = smelted_item
    ? `${smelted_item.count} ${smelted_item.name}`
    : `0 smelted items`;
  const input_name = input_item
    ? `${input_item.count} ${input_item.name}`
    : `0 input items`;
  const fuel_name = fuel_item
...
export async function attackNearest(bot, mobType, kill = true) {
  ...
  } // so it can go underwater. TODO: have an drowning mode so we don't turn off all self_preservation
  const mob = world
...
export async function defendSelf(bot, range = 9) {
  ...
  while (enemy) {
    ...
    if (bot.entity.position.distanceTo(enemy.position) <= 2) {
      try {
        bot.pathfinder.setMovements(new pf.Movements(bot));
        const inverted_goal = new pf.goals.GoalInvert(
...
export async function placeBlock(
  ...
  if (
    ...
    const inverted_goal = new pf.goals.GoalInvert(goal);
    bot.pathfinder.setMovements(new pf.Movements(bot));
...
export async function equip(bot, itemName) {
   ...
   **/
  const item = bot.inventory.slots.find(
    (slot) => slot && slot.name === itemName,
  );
  if (!item) {
...
export async function putInChest(bot, itemName, number_ = -1) {
  ...
  const to_put = number_ === -1 ? item.count : Math.min(number_, item.count);
  await goToPosition(
    bot,
    chest.position.x,
    chest.position.y,
...
export async function takeFromChest(bot, itemName, number_ = -1) {
  ...
  const to_take = number_ === -1 ? item.count : Math.min(number_, item.count);
...
export async function moveAway(bot, distance) {
  ...
  if (bot.modes.isOn("cheat")) {
    ...
    const last_move = path.path.at(-1);
    console.log(last_move);
    if (last_move) {
      const x = Math.floor(last_move.x);
      const y = Math.floor(last_move.y);
      const z = Math.floor(last_move.z);
      bot.chat("/tp @s " + x + " " + y + " " + z);
      return true;
    }
  }
...
export async function moveAwayFromEntity(bot, entity, distance = 16) {
   ...
   * @returns {Promise<boolean>} true if the bot moved away, false otherwise.
   **/
  const goal = new pf.goals.GoalFollow(entity, distance);
  const inverted_goal = new pf.goals.GoalInvert(goal);
  bot.pathfinder.setMovements(new pf.Movements(bot));
  await bot.pathfinder.goto(inverted_goal);
  return true;
